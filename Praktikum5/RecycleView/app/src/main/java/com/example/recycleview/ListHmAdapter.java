...
public class ListHmAdapter extends RecyclerView.Adapter<ListHmAdapter.ListViewHolder> {
    private ArrayList<mantan> listMantan;
    public ListHmAdapter(ArrayList<mantan>list){
    ...
    public void onBindViewHolder(@NonNull ListHmAdapter.ListViewHolder holder, int position) {
        mantan mantan = listMantan.get(position);
        Glide.with(holder.itemView.getContext())
                .load(mantan.getPhoto())
                .apply(new RequestOptions().override(55,55))
                .into(holder.imgPhoto);
        holder.tvName.setText(mantan.getName());
        holder.tvDesc.setText(mantan.getDescription());
    }
